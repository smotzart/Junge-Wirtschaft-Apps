...
var DetailShortModel = (function (_super) {
  ...
  Object.defineProperty(DetailShortModel.prototype, "event_date", {
    get: function () {   
      if (this.event.starts_at && this.event.ends_at) {
        var start = moment(this.event.starts_at).date() == moment(this.event.ends_at).date() ? "" : moment(this.event.starts_at).month() == moment(this.event.ends_at).month() ? moment(this.event.starts_at).format("D.-") : moment(this.event.starts_at).format("D. MMMM YYYY-");
